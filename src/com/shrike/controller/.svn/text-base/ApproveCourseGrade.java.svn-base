 ...
 */
@WebServlet("/ApproveCourseGrade.do")
public class ApproveCourseGrade extends HttpServlet {
	
	/** The Constant serialVersionUID. */
	private static final long serialVersionUID = 1L;
       
    /**
     * Instantiates a new approve course grade.
     *
     * @see HttpServlet#HttpServlet()
     */
    public ApproveCourseGrade() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * Do get method to load the approve course page 
	 *
	...
	/**
	 * Do post method to approve the stduent's course and notify that to student
	 *
	 * @param request the request
	 * @param response the response
	 * @throws ServletException the servlet exception
		...
		if (btnSubmit != null){
			...
			if(request.getParameter("list").equals("course")){
				for(int i = 1; i <= number; i++){
					...
					if(request.getParameter(""+i).contains("Approve")){
						cApprove.approveCourse(request.getParameter("id"+i));
					}
					if(request.getParameter(""+i).contains("Reject")){
						cApprove.rejectCourse(request.getParameter("id"+i));
					}
				}
				...
				for(int i = 1; i <= number; i++){
					...
					if(request.getParameter(""+i).contains("Reject")){
						cApprove.rejectGrade(request.getParameter("id"+i));
					}
				}
